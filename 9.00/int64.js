...
function Int64(low, high) {
    ...
    if (arguments.length > 2 || arguments.length == 0)
        throw TypeError("Numero incorrecto de argumentos para el constructor");
    if (arguments.length == 2) {
        if (typeof low != 'number' || typeof high != 'number') {
            throw TypeError("Ambos argumentos tienen que ser numeros");
        }
        if (low > 0xffffffff || high > 0xffffffff || low < 0 || high < 0) {
            throw RangeError("Ambos argumentos deben caber dentro de un uint32");
        }
        low = low.toString(16);
    ...
    switch (typeof low) {
        ...
        case 'object':
            if (low instanceof Int64) {
                ...
                if (low.length != 8)
                    throw TypeError("La matriz debe tener exactamente 8 elementos.");
                bytes.set(low);
            }
            break;
        case 'undefined':
    ...
    this.asDouble = function () {
        ...
        if (bytes[7] == 0xff && (bytes[6] == 0xff || bytes[6] == 0xfe))
            throw new RangeError("No puede ser representado por un doble");
    ...
    this.asInteger = function () {
        if (bytes[7] != 0 || bytes[6] > 0x20) {
            debug_log("ALGO MALO HA SUCEDIDO!!!");
            throw new RangeError(
                "No se puede representar como un numero regular");
        }
    ...
    this.asJSValue = function () {
        if ((bytes[7] == 0 && bytes[6] == 0) || (bytes[7] == 0xff && bytes[
            6] == 0xff))
            throw new RangeError(
                "No se puede representar como un JSValue");
    ...
    function operation(f, nargs) {
        return function () {
            if (arguments.length != nargs)
                throw Error("Argumentos insuficientes para la funcion " + f.name);
            var new_args = [];
